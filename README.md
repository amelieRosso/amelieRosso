## Hi there 👋

- 👨‍🎓 Junior at Cornell University, College of Engineering
- 📚 Majoring in Information Science & minoring in Artificial Intelligence

<h2>Notable Projects</h2>

### World Heritage Search (<a href="https://github.com/amelieRosso/4300-JSON-travel_match">GitHub</a>)

  - A natural language search platform focused on UNESCO World Heritage Sites<br>
  - Utilizes an SVD-based cosine similarity algorithm<br>
  - Designed information retrieval algorithms & implemented the cosine similarity algorithm<br>
  - Integrated a 3D map component using ArcGIS Maps SDK for JavaScript<br>

<img width="2791" height="1316" alt="image" src="https://github.com/user-attachments/assets/ad4631af-d74b-4f60-87af-671987b72dce" />
<img width="2738" height="1645" alt="image" src="https://github.com/user-attachments/assets/170ed627-62d8-4397-8b7b-86c3ad6fd769" />

### Digital Tech & Innovation's Team Website (<a href="https://cornelldti.org/">Website</a>, <a href="https://github.com/cornell-dti/idol/tree/main/new-dti-website-redesign">GitHub</a>)

  - Digital Tech & Innovation's external website showcasing the products, team, courses, and recruitment timeline<br>
  - Product Manager of the project, working to create a high-performance, accessible website<br>
  - Created the sponsor page graphic using React and Tailwind.<br>

<img width="394.67" height="524" alt="image" hspace="10" src="https://github.com/user-attachments/assets/632fb5f3-1fd8-4592-a493-80b8fd2ff863" />
<img width="214.35" height="524" alt="Screenshot_20251024-153545" src="https://github.com/user-attachments/assets/a3cf2de0-3bb0-438c-97b5-eb8b03596b02" />
